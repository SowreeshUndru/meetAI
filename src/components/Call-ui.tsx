...
export const CallUI = ({ customId, creatorId, userId }: Props) => {
  const call = useCall();
  ...
  const handleLeave = async () => {
    if (!call) return;
  //  if(show!=="ended")  await call.leave();
    setShow("ended");
  };
  ...
  return (
    <StreamTheme className="h-[100vh]">
      ...
      {show === "call" && <CallActive userId={userId} onLeave={handleLeave} customId={customId} creatorId={creatorId} />}
